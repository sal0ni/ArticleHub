...
<html lang="en">
  ...
  <body>
        ...
        <div class="container"><a class="navbar-brand text-dark" href="index.php">ArticleHub.com</a>
          ...
          <ul class="navbar-nav ml-auto">
            ...
            <?php } 
             ...
             <button class="btn btn-outline-dark" type="button" data-toggle="modal" data-target="#AchievementModal" onclick="upload();">Upload</button>&nbsp;
    ...
    <!--Main Section-->
    <section class="bg-cover bg-center mt-5" id="main" style="background: linear-gradient(#000, transparent), url('img/achievements.jpg') no-repeat center; 
background-size: cover; height: 20%;">
      <div class="container py-4 index-forward">
        <div class="row">
          <div class="col-lg-12 col-md-12 col-sm-12">
            <h2 class="display-4 text-center my-5 bolded text-white">Achievements</h2>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="container mt-5 mb-5">
      <header>
        <h1 class="text-dark bolded">Achievement Stories</h1>
        <p class="text-dark my-4"></p>
      ...
      <div class="container">
        ...
        <?php
              ...
              $sql = "Select id,title,description,image from achievement where status=2";
              $result = mysqli_query($conn, $sql);
              while($row = mysqli_fetch_assoc($result)) {
              ?>
            <div class="col-lg-3 col-md-6 col-sm-12 my-3">
            <div class="card">
              <img class="card-img-top" data-toggle="modal" data-target="#modal<?php $row['id']; ?>" src="images/<?php echo $row['image'] ?>" alt="Card image cap">
              <div class="card-body">
              <header>
                <h3 class="bolded"><?php echo $row['title'] ?></h3>
              </header>
                <p class="card-text"><?php echo $row['description'] ?></p>
                <p class="float-right font-italic"><?php $sql = "SELECT firstName, lastName,branch,year from student where enrollmentNo=(SELECT enrollmentNo FROM submission WHERE id=" . $row['id'] . " and category='" . "achievement')";
                      $student = mysqli_query($conn, $sql);
                      $row_s = mysqli_fetch_assoc($student);?></p>
                <p class="float-right font-italic">- <?php echo $row_s['firstName'] . " " . $row_s['lastName'] . ", " . $row_s['year'] . " Year " .  $row_s['branch']?></p>
              </div>
            </div>
            </div>    
            <?php } ?>
<!--
              <div class="col-lg-3 col-md-6 col-sm-12 my-3">
                <div class="card">
                  <img class="card-img-top" data-toggle="modal" data-target="#exampleModalCenter" src="img/bg.jpg" alt="Card image cap">
                  <div class="card-body">
                  <header>
                    <h3 class="bolded">Lorem Ipsum</h3>
                  </header>
                    <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                    <p class="float-right font-italic">- Rahul</p>
                  </div>
                </div>
              </div>
              -->
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalTitle">Achievements</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
      <form action="upload_achievement.php" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <input type="text" name="event" class="form-control" placeholder="Event" id="recipient-name">
          </div>
          <div class="form-group">
            <input type="text" name="description" class="form-control" placeholder="Description" id="recipient-name">
          </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <input type="submit" name="upload_achievement" value="Upload" class="btn btn-dark">
      </div>
      </form>
    </div>
  </div>
